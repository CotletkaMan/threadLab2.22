...
#include "system/Components/Elasity.cpp"
#include "system/Components/FirstConstrain.cpp"
#include "system/Components/Force.cpp"
...
Plate* createPlate(int x , int y , double** voltage){
	...
	for(int node = 1 ; node <= (x * y) ; node++){
		if(node % x != 0){
			plate -> addComponent(node , node + 1 , new Elasity(C * C, dx));
		}
		else{
			plate -> addComponent(0 , node , new FirstConstrain(0));
			plate -> addComponent(node , 0 , new Elasity(C * C, dx));
			plate -> addComponent(0 , node - x + 1 , new Elasity(C * C, dx));
			plate -> addComponent(0 , node - x + 1 , new FirstConstrain(0));
		}
		if((node - 1) / x < y - 1){
			plate -> addComponent(node , node + x , new Elasity(C * C , dy));
			plate -> addComponent(0 , node , new TimeElasity(0));
		}
		else{
			plate -> addComponent(0 , node , new FirstConstrain(0)); 
			plate -> addComponent(node , 0 , new Elasity(C * C, dy));
			plate -> addComponent(0 , (node - 1) % x + 1, new Elasity(C * C, dy));
			plate -> addComponent(0 , (node - 1) % x + 1, new FirstConstrain(0));
		}

		plate -> addComponent(0 , x + 1 , new Force(dx , dy , force));
	}
	return plate;
}
